	...
	def auth = {
		...
		else if (cUser!=null)
		{
			...
			if (userInBd == null) {
				...
				//code added by hari start

				
				if (deploymentGroup != null) { // if deployment group is defined
			...
			//code added by hari end
		}	
		
		/*for(String i:userList){
							
							if(i == userId){
								userInDeploymentGroup =true;
								redirect uri: SpringSecurityUtils.securityConfig.successHandler.defaultTargetUrl
								return
...
/*
						deploymentGroup.members.each { // go through members of the group and see if current user is part of the group
							currUser ->

							if (currUser.id == userInBd.id) { // current user is in deployment group, so he can login normally
								userInDeploymentGroup = true;
								redirect uri: SpringSecurityUtils.securityConfig.successHandler.defaultTargetUrl
								return
							}
						...
						/*if (!userInDeploymentGroup) { // if not in group then forward to deployment page.
					...
					/*render view: 'Deployment'//, model: [postUrl: request.contextPath + SpringSecurityUtils.securityConfig.apf.filterProcessesUrl]
					return*/
